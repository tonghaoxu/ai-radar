...
**⚠ 这个页面里所有 effect 守卫都必须对「重复执行」幂等。** app router 从 Next 13.5.1 起默认开启 React 严格模式，开发环境下 effect 会跑两遍（挂载 → 清理 → 再挂载）。`useRef(true)` 这种一次性守卫会被第一遍消费掉，第二遍直接掉进本该跳过的分支。踩过两次，都表现为「返回后闪回顶部约 1 秒再跳回原位」：

- 加载守卫走空 → 第二遍执行非静默的 `fetchArticles()` → 列表被 loading 转圈清空，页面高度从两万像素塌回几百，滚动位置归零
- 回顶部守卫走空 → 第二遍直接 `window.scrollTo(0, 0)`，把刚恢复好的位置顶回去

现在两处都改成记录**筛选条件指纹**（`filterKey()`）——「这组数据是不是已经在手上」「这组条件是不是已经归过位」，重复执行天然幂等。判据要用状态而不是次数。
